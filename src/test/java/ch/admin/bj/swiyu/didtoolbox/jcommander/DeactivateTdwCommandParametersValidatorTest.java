...
class DeactivateTdwCommandParametersValidatorTest {
    ...
    private static JCommander buildDeactivateCommand() {
        return JCommander.newBuilder()
                .addCommand("deactivate", new DeactivateTdwCommand())
                .build();
    }
    ...
    private static void testValidatePrimusParameters() {
        ...
        assertDoesNotThrow(() -> {
            buildDeactivateCommand().parse("deactivate", "-d", dummyDidLogFile.getPath(), // required
    ...
    private static void assertCreateTdwCommandThrowsParameterException(String... args) {
        var argz = Stream.concat(
                        Arrays.stream(new String[]{"deactivate", "-d", dummyDidLogFile.getPath()}), // required
                        Arrays.stream(args))
                .toArray(size -> (String[]) Array.newInstance(String.class, size));

        assertThrowsParameterException(() -> buildDeactivateCommand().parse(argz),
